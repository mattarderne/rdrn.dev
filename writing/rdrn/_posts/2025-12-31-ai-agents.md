...
**Update (2026-02-18):** Anthropic published [Advanced Tool Use](https://www.anthropic.com/engineering/advanced-tool-use) which formalises three patterns for scaling tool use: (1) a **Tool Search Tool** for dynamic discovery via `defer_loading` instead of loading all tools upfront, (2) **Programmatic Tool Calling** where the model writes code to call tools and process results outside its context, and (3) **Tool Use Examples** to teach correct invocation patterns. Combined with effort control and context compaction, these boosted Opus 4.5 deep research performance by ~15pp.
{: .note}
